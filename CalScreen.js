...
import { View, Text, TouchableOpacity, TextInput, FlatList, StyleSheet, ScrollView, Image } from 'react-native';
...
const CropRotationCalendar = () => {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Text style={styles.rotationDateText}>{i18next.t('nextRotationDate')} {rotationDate.toDateString()}</Text>
      ...
      <View style={styles.calendarContainer}>
        <Calendar
          markedDates={{
            [rotationDate.toISOString().slice(0, 10)]: { selected: true, selectedColor: 'blue' },
          }}
          hideExtraDays
          theme={{
      ...
      <View style={styles.addedCropsContainer}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Text style={styles.addedCropText}>{item.name} - {i18next.t('rotationInterval')}: {item.rotationInterval} {i18next.t('months')}</Text>
          )}
        />
      </View>
    </ScrollView>
...
const LanguageSelector = () => {
  ...
  return (
    <View style={styles.languageSelectorContainer}>
      ...
      <TouchableOpacity style={styles.languageButton} onPress={() => changeLanguage('hi')}>
        <Text style={styles.languageButtonText}>हिन्दी</Text>
      </TouchableOpacity>
      <TouchableOpacity style={styles.languageButton} onPress={() => changeLanguage('te')}>
